...
public abstract class Bag {
    ...
    public String popItem() {
        if (this.numberOfContents == 0) {
            ...
            String store = this.contents[this.numberOfContents - 1];
            String[] store2 = new String[this.contents.length - 1];
            for(int i = 0; i < store2.length; i++) {
                store2[i] = this.contents[i];
            }
            this.contents = store2;
            this.numberOfContents--;
            return store;
        }
    }
    ...
    public void increaseCapacity(int n) {
        ...
        String[] store = this.contents.clone();
        this.contents = new String[this.capacity + n];
        for (int i = 0; i < this.capacity; i++) {
            this.contents[i] = store[i];
        }
        this.capacity += n;
    }
